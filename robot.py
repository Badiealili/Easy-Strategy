...
class Robot:
    def __init__(self, canvas: canvas.Canvas, starting_pos: tuple[int, int], starting_angle: int, width: int, height: int) -> None:
        ...
        # self.id = self.canvas._canvas.create_rectangle(self.x - self.width // 2, self.y - self.height // 2, self.x + self.width // 2, self.y + self.height // 2, fill="green")
    ...
    def update(self, x: int, y: int, angle: int) -> None:
        ...
        self.y = y
        self.angle = angle
        self.canvas._canvas.delete(self.id)
        ...
        self.id = self.canvas._canvas.create_polygon(x1, y1, x2, y2, x3, y3, x4, y4, fill="green")
    ...
    def rotate(self, angle: int, coords) -> list[tuple[int, int]]:
        """Rotate the robot rectangle with *angle* degrees around its center"""
